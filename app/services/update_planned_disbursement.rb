  ...
  def call(attributes: {})
    ...
    if attributes.key?(:financial_quarter) & attributes.key?(:financial_year)
      ...
      planned_disbursement.financial_quarter =
        FinancialPeriods.financial_quarter_from_date(attributes.fetch(:period_start_date).to_date)
      planned_disbursement.financial_year =
        FinancialPeriods.financial_year_from_date(attributes.fetch(:period_start_date).to_date)
    end
