  ...
  class ImportFromCsv
    ...
    def create_activity(row, index)
      if row["RODA ID Fragment"].present? && row["Parent RODA ID"].present?
        creator = ActivityCreator.new(user: @uploader, row: row, delivery_partner_organisation: @delivery_partner_organisation)
        creator.create
    ...
    def update_activity(row, index)
      if row["RODA ID Fragment"].present?
        ...
        updater = ActivityUpdater.new(row: row, user: @uploader, delivery_partner_organisation: @delivery_partner_organisation)
        updater.update
    ...
    class ActivityUpdater
      ...
      def initialize(row:, user:, delivery_partner_organisation:)
        @errors = {}
        ...
        if @activity && !ActivityPolicy.new(@user, @activity).update?
    ...
    class ActivityCreator
      ...
      def initialize(user:, row:, delivery_partner_organisation:)
        @user = user
        @organisation = user.organisation
        @delivery_partner_organisation = delivery_partner_organisation
        @errors = {}
        ...
        if @parent_activity && !ActivityPolicy.new(@user, @parent_activity).create_child?
